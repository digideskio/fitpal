class MFP
  ...
  def self.training_day_macros
    ...
    protein = (bodyweight_in_lbs * Tdee.bodyfat_multiplier) * (ENV['PROTEIN_PER_LBS'].to_f || 1.3)
    protein_calories = protein * 4
    tdee_calorie_goal = Tdee.find_by(date: Date.today).tdee * 1.2 # sedentary = 1.2 multiplier
    calorie_goal = tdee_calorie_goal * (ENV['TRAINING_MULTIPLIER'].to_f || 1.1)
  ...
  def self.rest_day_macros
    ...
    protein_calories = protein * 4
    tdee_calorie_goal = Tdee.find_by(date: Date.today).tdee * 1.2 # sedentary = 1.2 multiplier
    calorie_goal = tdee_calorie_goal * (ENV['REST_MULTIPLIER'].to_f || 0.8)
    calories_without_protein = (calorie_goal - protein_calories).to_f
